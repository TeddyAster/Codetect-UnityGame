...
public class ProGifManager : MonoBehaviour
{
	...
	private void _InitRecorder(Camera targetCamera, bool autoClear)
	{
		...
		m_GifRecorder.recorderCom.m_Comments = m_Comments;
	...
	/// </summary>
	public void SetCommentExtension(string comments)
    {
		...
		m_GifRecorder.recorderCom.m_Comments = comments;
	}
	...
	public void PlayGif(Image playerImage, Action<float> onLoading = null)
	{
		...
		m_GifPlayer.debugLog = m_DebugLog;
	...
	/// <param name="playerImage">Target image for displaying gif.</param>
	/// <param name="onLoading">On loading. Return value: loading progress(float)</param>
	public void PlayGif(byte[] bytes, Image playerImage, Action<float> onLoading = null)
	{
		Clear();
		m_GifPlayer = new ProGifPlayer();
		m_GifPlayer.debugLog = m_DebugLog;
		m_GifPlayer.SetDecodeSettings(m_Decoder, m_DecodeMode, m_TargetDecodeFrameNum, m_FramePickingMethod, m_OptimizeMemoryUsage);
		m_GifPlayer.Play(bytes, playerImage);
		m_GifPlayer.SetLoadingCallback((progress) => {
			//Check progress
			if (progress >= 1f)
			{
	...
	/// <param name="playerRawImage">Target RawImage for displaying gif.</param>
	/// <param name="onLoading">On loading. Return value: loading progress(float)</param>
	public void PlayGif(byte[] bytes, RawImage playerRawImage, Action<float> onLoading = null)
	{
		Clear();
		m_GifPlayer = new ProGifPlayer();
		m_GifPlayer.debugLog = m_DebugLog;
		m_GifPlayer.SetDecodeSettings(m_Decoder, m_DecodeMode, m_TargetDecodeFrameNum, m_FramePickingMethod, m_OptimizeMemoryUsage);
		m_GifPlayer.Play(bytes, playerRawImage);
		m_GifPlayer.SetLoadingCallback((progress) => {
			//Check progress
			if (progress >= 1f)
			{
	...
	/// <param name="playerRenderer">Target renderer for displaying gif.</param>
	/// <param name="onLoading">On loading. Return value: loading progress(float)</param>
	public void PlayGif(byte[] bytes, Renderer playerRenderer, Action<float> onLoading = null)
	{
		Clear();
		m_GifPlayer = new ProGifPlayer();
		m_GifPlayer.debugLog = m_DebugLog;
		m_GifPlayer.SetDecodeSettings(m_Decoder, m_DecodeMode, m_TargetDecodeFrameNum, m_FramePickingMethod, m_OptimizeMemoryUsage);
		m_GifPlayer.Play(bytes, playerRenderer);
		m_GifPlayer.SetLoadingCallback((progress) => {
			//Check progress
			if (progress >= 1f)
			{
	...
	public void SetPlayerOnGifError(string playerName, Action<string> onGifError)
	{
		...
		m_GifPlayer.SetOnGifErrorCallback(onGifError);
	}
