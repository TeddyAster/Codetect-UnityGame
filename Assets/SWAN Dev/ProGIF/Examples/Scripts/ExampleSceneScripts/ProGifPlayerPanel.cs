...
public class ProGifPlayerPanel : MonoBehaviour
{
	...
	/// <summary> If 'true', try decode (and play) the GIF even if the file is broken. (Try to play the good frames) </summary>
	...
	public DataPathType localDebugDataPath = DataPathType.UrlInputField;
	...
	public void Play()
	{
		...
		// Find and select the path type (for testing in Unity Editor only, may require more handling for some platforms)
		localDebugDataPath = _CheckAndSelectPathType(ref gifPath);

		switch (localDebugDataPath)
		{
			case DataPathType.UrlInputField:
				break;
		...
		if (localDebugDataPath != DataPathType.UrlInputField && !gifPath.ToLower().Contains(".gif")) gifPath += ".gif";
	...
	/// Find and select the path type (for testing in Unity Editor only, may require more handling for some platforms)
	/// </summary>
	private DataPathType _CheckAndSelectPathType(ref string gifPath)
    {
		string pathLower = gifPath.ToLower();
		if (pathLower.StartsWith("assets/streamingassets/"))
		{
			gifPath = gifPath.Substring("assets/streamingassets/".Length);
			return DataPathType.StreamingAssetsPath;
		}
		else if (pathLower.StartsWith("assets/"))
		{
			gifPath = gifPath.Substring("assets/".Length);
			return DataPathType.DataPath;
		}
		else if (pathLower.StartsWith("http"))
		{
			return DataPathType.UrlInputField;
		}
		else
		{
			return DataPathType.PersistentDataPath;
		}
	}

	public System.Collections.Generic.List<Texture2D> m_Textures = new System.Collections.Generic.List<Texture2D>();
...
//				displayHandler.SetImage(m_GifImage, temp);
			...
			Debug.Log("CommentExtension: " + firstFrame.comments);
...
//				}
	...
	public void GetGifInfoTest(RawImage image)
	{
		PGif.GetGifInfo(input_Url.text, (firstFrame) =>
		{
			image.texture = firstFrame.gifTexture.GetTexture2D();
			image.SetNativeSize();
		}, ProGifPlayerComponent.Decoder.ProGif_QueuedThread);
	}

	public void GetGifInfoTest(Image image)
	{
		PGif.GetGifInfo(input_Url.text, (firstFrame) =>
		{
			image.sprite = firstFrame.gifTexture.GetSprite();
			image.SetNativeSize();
		}, ProGifPlayerComponent.Decoder.ProGif_QueuedThread);
	}
}
